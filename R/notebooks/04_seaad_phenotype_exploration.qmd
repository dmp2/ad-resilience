---
title: "SEA-AD phenotype landscape: pathology, cognition, APOE, regional tissue state and glial molecular state"
subtitle: "Five composite figures that establish what the cohort can and cannot support for the AD-resilience study"
format:
  html:
    toc: true
    toc-depth: 2
...
The study asks whether donors with comparable Alzheimer's pathology differ in cognition, and whether
regional tissue state and glial molecular state help to explain that difference. Before any
morphology is added, this notebook establishes five preliminary observations.

1. Does SEA-AD contain pathology–cognition discordance at all?
2. Does APOE shift pathology, cognition at comparable pathology, or both?
3. In which regions do donors with comparable local pathology differ in cognition?
4. How do regional pathology and tissue response vary with stage?
5. Do the two carried-forward molecular-state variables track stage, cognition, or neither?

Everything here is descriptive and cross-sectional. The residual used below is an **exploratory
phenotype**, not a validated measure of resilience. No association here is causal, and no ROI or
signature is excluded or reselected because of it.

Inputs are reused unchanged:

- QNP P/H block: `qnp_primary_long()` from notebook 00, Grey matter only.
- Molecular state: the competitive scores in `molecular_state_analysis_table.csv` from notebook 03.
- Donor metadata and SpecimenMetadata.csv: the sources pinned in notebook 00.

Nothing was downloaded, rescored or re-aggregated.
...
## Figure 1: the pathology–cognition landscape
...
**Reading.** The ACT donors with CASI contain the discordance the study needs, within the
restricted population described below. CASI falls with CPS (ρ =
`r sprintf("%.2f", rho_casi_cps)`), but among the `r sum(casi_set$cps > 0.6)` donors with CPS
above 0.6 it still spans `r min(casi_set$casi[casi_set$cps > 0.6])` to
`r max(casi_set$casi[casi_set$cps > 0.6])`. Among
Braak V–VI donors, `r stage_mix$n[stage_mix$cog_status == "No dementia"]` of
`r sum(stage_mix$n)` have a documented status of no dementia. The baseline model explains
R² = `r sprintf("%.2f", casi_r2)` of CASI, almost entirely through CPS. Age, sex, education and
the assessment interval add little at this n.

The trajectories in D separate after baseline, not at it. First-visit memory, a median of
`r median(long_first$first_ytd)` years before death, is unrelated to the residual (ρ =
`r sprintf("%.2f", rho_long["mem_first"])`; first-visit executive function ρ =
`r sprintf("%.2f", rho_long["exf_first"])`). Memory slope tracks it (ρ =
`r sprintf("%.2f", rho_long["slope_mem"])`), and executive-function slope less so (ρ =
`r sprintf("%.2f", rho_long["slope_exf"])`). Two things limit this reading:

- **The slope link is partly mechanical.** Slope and terminal CASI share the last assessments.
- **Baseline memory has a ceiling.** `r n_at_ceiling` of `r nrow(long_first)` donors sit at the
  maximum first-visit memory score (`r sprintf("%.2f", mem_ceiling)`), so baseline reserve is
  poorly resolved.

Biennial visits and whole-year ages also cannot distinguish slower decline from later-onset
decline.

<details><summary>Residual QC (no figure)</summary>

- **Residual against CPS:** Spearman ρ = `r sprintf("%.2f", resid_qc$rho_cps)`.
- **Residual against the CASI-to-death interval:** ρ = `r sprintf("%.2f", resid_qc$rho_interval)`.
- **Mean residual by CPS tertile:**
  `r paste(sprintf("%+.1f", resid_qc$tertile_means), collapse = " / ")` points. The middle tertile
  sits high.
- **Quadratic CPS term:** adding it raises R² by `r sprintf("%.3f", resid_qc$quad_delta_r2)`
  (F-test p = `r sprintf("%.2f", resid_qc$quad_p)`). It is not adopted, but the linear residual
  may carry some curvature in CPS.
- **Ceiling:** `r resid_qc$n_casi_ge95` of `r nrow(casi_set)` donors score CASI ≥ 95, so the
  positive residual tail is truncated.
- **Cohort:** not in the model. All CASI donors are ACT, so it is constant.

</details>

### Who the CASI analyses represent
...
The CASI analyses cover a restricted and systematically different part of the cohort. They are
the ACT donors: older (median age at death `r sel$age_act` vs `r sel$age_adrc`) and less
advanced (median CPS `r sprintf("%.2f", sel$cps_act)` vs `r sprintf("%.2f", sel$cps_adrc)`). No
ADRC donor is at Braak 0–III, and `r sel$b6_adrc` of the 15 Braak VI donors are ADRC.

The advanced, younger, ε4-enriched end of SEA-AD is therefore under-represented in every CASI
panel: `r sel$carriers_adrc` of 25 ε4 carriers and `r sel$e44_adrc` of 6 ε4/ε4 donors are ADRC.
Figures 1A, 1C, 2C–D, 3 and the right column of Figure 5 describe pathology–cognition
discordance in older ACT donors, mostly at Braak IV–V. They do not describe the full cohort.

---

## Figure 2: APOE and susceptibility, pathology and cognition
...
**Reading.** APOE differences appear principally in pathology burden. ε4 carriers have higher CPS
(median `r sprintf("%.2f", cps_by_apoe$median[2])` vs
`r sprintf("%.2f", cps_by_apoe$median[1])`, Wilcoxon p = `r format(cps_wilcox$p.value, digits = 2, scientific = TRUE)`,
descriptive). They are concentrated in Braak V–VI. The `r n44` ε4/ε4 donors have median CPS
`r sprintf("%.2f", med44)`, and only `r n44_casi` of them have CASI.

Carriers are almost absent from the lowest CPS tertile, so cognition can be compared at
comparable pathology only across the middle and upper tertiles. Within that range the carrier and
noncarrier trends overlap. The adjusted-cognition difference is small and uncertain:

- **All carriers:** `r fmt_ci(resid_diff["est"], resid_diff["lo"], resid_diff["hi"], 1)` points.
- **Excluding ε2/ε4:** `r fmt_ci(resid_diff_no24["est"], resid_diff_no24["lo"], resid_diff_no24["hi"], 1)`.
- **Within the shared CPS range:** `r fmt_ci(resid_diff_support["est"], resid_diff_support["lo"], resid_diff_support["hi"], 1)`.

There is therefore no evidence here for an APOE association with cognition conditional on
pathology. There is also no power to exclude one, and nothing here bears on protection.

**Selection affects this comparison.** The CPS difference is not an artefact of pooling cohorts:
within ACT alone, carriers still have higher CPS (median
`r sprintf("%.2f", sel$cps_carrier_act)` vs `r sprintf("%.2f", sel$cps_non_act)`). The
cognition comparison (C–D), however, loses `r sel$carriers_adrc` of the 25 carriers and
`r sel$e44_adrc` of the 6 ε4/ε4 donors, all ADRC and all without CASI. It therefore concerns
mainly ε3/ε4 donors surviving to old age in ACT.

---

## Figure 3: is regional resilience identifiable?
...
**Comparable-burden check.** A wide CASI range inside an AT8 tertile is not the same as
discordance at matched pathology. The tertiles span up to two orders of magnitude of AT8, and
donors in the same tertile can differ widely in CPS. So each ROI is also checked on donor
*pairs* that are close on both global and local burden.

- **Tolerances**, declared once and not varied:
  - |ΔCPS| ≤ 0.05;
  - local AT8 within a factor of 2, i.e. |Δlog₁₀(AT8 + 0.001)| ≤ 0.30;
  - local Aβ within a factor of 2 in the sensitivity column.
- **Elevated burden:** only pairs where both donors have CPS above the CASI-set median.
- **Discordant pair:** |ΔCASI| ≥ 10 points, about `r sprintf("%.1f", 10 / sigma(casi_fit))` residual SDs of the baseline model. This
  describes a difference; it is not a diagnostic cut-off.
- **Reference:** the median |ΔCASI| over *all* elevated-burden pairs, so matching can be seen to
  shrink the spread or not.
- **Reporting unit:** pairs share donors, so the table reports distinct donors, not pairs.
...
**Reading.** The regional data establish three different things, and only the second is
pathology-matched discordance.

1. **Broad spread at elevated pathology.** In the upper AT8 tertile, CASI spans
   `r min(f3_bins$casi_max[f3_bins$at8_tertile == 3] - f3_bins$casi_min[f3_bins$at8_tertile == 3])`
   to `r max(f3_bins$casi_max[f3_bins$at8_tertile == 3] - f3_bins$casi_min[f3_bins$at8_tertile == 3])`
   points in every ROI. On its own this is not matched pathology.
2. **Discordance at comparable measured burden.** At elevated burden, most donors have a partner
   within the declared CPS and local-AT8 tolerances, and matching narrows the spread little (most
   in DFC):
   median |ΔCASI| is `r sprintf("%.0f", min(comparable$median_matched))`–`r sprintf("%.0f", max(comparable$median_matched))`
   points matched vs `r sprintf("%.0f", median(comparable$median_all))` unmatched.
   `r min(comparable$disc_donors)`–`r max(comparable$disc_donors)` donors per ROI sit in a matched
   pair that differs by at least 10 CASI points. This survives matching on local Aβ as well
   (`r min(comparable$disc_ab_donors)`–`r max(comparable$disc_ab_donors)` donors); DFC has the
   fewest.
3. **Insufficient support once molecular state is required.** A matched discordant pair with
   MG4 and astrocyte scores on both donors involves `r cmp("MTG", "disc_T_donors")`,
   `r cmp("DFC", "disc_T_donors")` and `r cmp("MEC", "disc_T_donors")` donors in MTG, DFC and MEC,
   but only `r cmp("STG", "disc_T_donors")`, `r cmp("HIP", "disc_T_donors")` and
   `r cmp("V1C", "disc_T_donors")` in STG, HIP and V1C.

The matched spread does not show that the discordance is biological. Terminal CASI also carries
test measurement error and a variable CASI-to-death interval, and the measured burden is
grey-matter percent area in one sampled block per ROI.

By position on the pathology axis:

- **MTG and DFC** place their spread in the middle-to-upper AT8 range.
- **MEC and HIP** place it entirely at high burden, which also makes them informative for injury.
- **V1C** has pTau near the detection floor for most donors.

None of these readings is a reason to drop a region from later morphology.

---

## Figure 4: pathology and tissue integrity across disease stage
...
**Reading.** The two pathologies follow different stage patterns:

- **Aβ** is already present in MEC at Braak 0–III, and rises most in the neocortex (MTG, DFC).
- **pTau** rises steeply in MEC from the earliest stages and reaches the neocortex mainly at
  Braak V–VI.
- **NeuN** is lower at Braak VI than at IV–V in all three regions, most clearly in MEC (median
  `r round(mec_neun$neun[mec_neun$braak_group == "VI"])` vs
  `r round(mec_neun$neun[mec_neun$braak_group == "IV"])` cells/mm² at Braak IV). NeuN varies
  widely within every stage, and correlates only weakly with local pTau in every region (ρ
  `r sprintf("%.2f", min(all_roi_stage[["NeuN vs local pTau"]], na.rm = TRUE))` to
  `r sprintf("%.2f", max(all_roi_stage[["NeuN vs local pTau"]], na.rm = TRUE))` across the
  `r sum(!is.na(all_roi_stage[["NeuN vs local pTau"]]))` regions with NeuN).
- **GFAP** is heterogeneous rather than uniformly progressive. It rises through Braak VI in MTG
  (median `r sprintf("%.1f", mtg_gfap$gfap[1])` → `r sprintf("%.1f", mtg_gfap$gfap[4])`%) and in
  MEC, but has no consistent stage trend in DFC (ρ with Braak
  `r sprintf("%.2f", all_roi_stage$GFAP[all_roi_stage$brain_region == "DFC"])`).

So at a given stage, donors and regions differ in tissue response, which is the heterogeneity
morphology is meant to explain.

<details><summary>All regions: Spearman correlation of each QNP measure with Braak stage</summary>
...
## Figure 5: molecular state, disease response or cognition-related variation?
...
**Reading.** Both states track disease stage, and they relate to the other variables differently:

- **With stage:** both rise with CPS in all three ROIs. The mean within-ROI ρ is
  `r fmt_pooled("MG4", "CPS")` for MG4 and `r fmt_pooled("Astro", "CPS")` for astrocyte
  activation (donor-cluster bootstrap).
- **With local pathology:** MG4 tracks local AT8 more closely than CPS (ρ
  `r sprintf("%.2f", min(rho_tbl$rho[rho_tbl$score == "MG4" & rho_tbl$covariate == "Local AT8"]))` to
  `r sprintf("%.2f", max(rho_tbl$rho[rho_tbl$score == "MG4" & rho_tbl$covariate == "Local AT8"]))`).
- **With adjusted cognition (descriptive, Figure 5 right column):** MG4 is modestly *lower* in
  donors with better-than-expected cognition, in the same direction in all three ROIs (mean ρ
  `r fmt_pooled("MG4", "Adjusted cognition")`). Astrocyte activation shows no such relation
  (`r fmt_pooled("Astro", "Adjusted cognition")`), and neither does the MG8 sensitivity variable.
- **With tissue injury:** astrocyte activation is the one that associates with lower NeuN density
  in all three ROIs; MG4 does not (table below).

APOE does not materially change either score at fixed CPS: every carrier contrast has a
confidence interval that includes zero. No stratified panel was therefore added.

### Joint-model check

A correlation with a residual can mislead when the molecular state is itself correlated with a
covariate the residual removed, as MG4 is with CPS. So each score is also entered directly into
the baseline model, one ROI and one score at a time:

`CASI ~ CPS + age + sex + education + CASI-to-death interval + state`

Details of the fit:

- **Sample:** baseline and joint model are fitted on the identical donors (ACT donors with CASI
  and that score in that ROI).
- **Scaling:** the state is standardised within that sample, so the estimate is CASI points per
  SD.
- **No nonlinear terms:** the Figure 1 QC found only a marginal quadratic signal (ΔR²
  `r sprintf("%.3f", resid_qc$quad_delta_r2)`), which is not a material misspecification.
- **No interactions** are fitted.
- **Co-pathology column:** the right-hand columns repeat the fit with LATE-NC stage 2–3 and
  severe arteriolosclerosis added to both the baseline and the joint model (see the co-pathology
  section).
...
The descriptive residual correlation largely holds up in the joint model, but not uniformly:

- **MG4, MTG and DFC:** CASI is lower per SD of MG4, `r jci("MTG", "MG4")` points in MTG and
  `r jci("DFC", "MG4")` in DFC.
- **MG4, MEC:** `r jci("MEC", "MG4")`, which includes zero.
- **Astrocyte activation:** no estimate excludes zero in any ROI.
- **Size:** adding MG4 raises R² by only
  `r sprintf("%.3f", min(joint_tbl$d_r2[joint_tbl$score == "MG4"]))`–`r sprintf("%.3f", max(joint_tbl$d_r2[joint_tbl$score == "MG4"]))`.

With LATE-NC and severe arteriolosclerosis added, the MG4 estimates become
`r jci("MTG", "MG4", "_c")` (MTG), `r jci("DFC", "MG4", "_c")` (DFC) and
`r jci("MEC", "MG4", "_c")` (MEC). They stay negative but attenuate by
`r sprintf("%.0f", 100 * min(1 - joint_tbl$est_c[joint_tbl$score == "MG4"] / joint_tbl$est[joint_tbl$score == "MG4"]))`–`r sprintf("%.0f", 100 * max(1 - joint_tbl$est_c[joint_tbl$score == "MG4"] / joint_tbl$est[joint_tbl$score == "MG4"]))`%,
and the interval now excludes zero only in MTG. These are small single-ROI models. In
the separate local-AT8 sensitivity (supporting estimates below), the MG4 interval includes zero in
DFC and MEC.

The estimates are pathology- and covariate-adjusted associations in older ACT donors. They say
nothing about MG4 protecting or harming cognition, and they are not grounds to reselect either
signature.

<details><summary>Supporting estimates: tissue-injury associations, MG8 sensitivity and APOE</summary>
...
## Supporting check: co-pathology and APOE context
...
Co-pathology is common in every phenotype group, and it is not balanced between them. Among
Braak V–VI donors, LATE-NC stage 2–3 is `r sprintf("%+.0f", 100 * late_diff)` percentage points
more frequent with dementia than without. Limbic or neocortical Lewy body disease is
`r sprintf("%+.0f", 100 * lewy_diff)` points more frequent.

In the CASI set, donors with worse-than-expected cognition carry more LATE-NC
(`r sprintf("%+.0f", 100 * late_resid)` points) and more severe arteriolosclerosis
(`r sprintf("%+.0f", 100 * arterio_resid)` points). Moderate–severe CAA is
`r sprintf("%+.0f", 100 * caa_apoe)` points more frequent in ε4 carriers, as expected for APOE ε4.
This is a second pathology axis on which the APOE groups differ.

The groups are small (15 no-dementia donors at Braak V–VI), so these are imbalances, not
estimates.

### Co-pathology model sensitivity
...
**Source check.** The fields are `LATE` and `Arteriolosclerosis` from the donor-metadata workbook.
In the CASI set, LATE counts are
`r paste(sprintf("%s %d", names(late_counts), late_counts), collapse = ", ")`: the one
unclassifiable donor is ADRC, so no CASI donor is lost. Arteriolosclerosis counts are
`r paste(sprintf("%s %d", names(art_counts), art_counts), collapse = ", ")`. Neither field has
missing values. Every LATE × arteriolosclerosis cell is populated (smallest `r min(cross)`), so the
two indicators are estimable.

**Results.** Adding the two indicators raises R² from
`r sprintf("%.2f", summary(cp_base)$r.squared)` to `r sprintf("%.2f", summary(cp_sens)$r.squared)`.
LATE-NC stage 2–3 is associated with `r term_ci(cp_sens, "late_2_3TRUE")` CASI points, and severe
arteriolosclerosis with `r term_ci(cp_sens, "arteriolo_severeTRUE")`. The CPS coefficient barely
moves.

The exploratory phenotype is kept as defined; its co-pathology-adjusted counterpart correlates
with it at r = `r sprintf("%.2f", cp_resid_cor)`. So co-pathology explains some of the
pathology-adjusted cognitive variation without redefining it. The MG4 estimates under this
specification are in the joint-model table (Figure 5).

---

## Synthesis

1. **Population.** Every cognition result describes the 69 ACT donors: older (median age at
   death `r sel$age_act`) and less advanced than the ADRC donors. The CASI set excludes
   `r sel$b6_adrc` of 15 Braak VI donors and `r sel$carriers_adrc` of 25 ε4 carriers, so the
   young, advanced, ε4-enriched end of SEA-AD is not represented. APOE differs mainly in
   pathology burden, a difference that holds within ACT alone.
2. **Regional discordance.** Broad CASI spread at elevated pathology is present in all six ROIs.
   So is discordance of ≥ 10 points between donors matched on CPS and local AT8, and it survives
   Aβ matching. What limits the ROIs is joint molecular support: it is adequate in MTG, DFC and
   MEC, limited or absent in STG, HIP and V1C.
3. **MG4 in the joint model.** The inverse MG4–CASI association persists in MTG and DFC. In MEC
   the interval includes zero. It adds
   `r sprintf("%.3f", min(joint_tbl$d_r2[joint_tbl$score == "MG4"]))`–`r sprintf("%.3f", max(joint_tbl$d_r2[joint_tbl$score == "MG4"]))`
   to R².
4. **Co-pathology sensitivity.** With LATE-NC and severe arteriolosclerosis added, MG4 stays
   negative in all three ROIs but attenuates, and its interval excludes zero only in MTG. The
   association is therefore partly sensitive to LATE and vascular pathology. Those two indicators
   themselves raise R² from `r sprintf("%.2f", summary(cp_base)$r.squared)` to
   `r sprintf("%.2f", summary(cp_sens)$r.squared)`.
5. **Astrocyte activation** has no adjusted cognition association in any ROI. It remains
   primarily a tissue-integrity association (lower NeuN) that rises with stage.
6. **Implications for morphology.**
   - Start in MTG, DFC and MEC, where matched discordance and molecular state coexist.
   - Keep CPS, local AT8 and the two co-pathology indicators as adjustment covariates.
   - Treat MG4 as a pathology-linked covariate whose cognitive association needs confirmation,
     not as a protective factor.
   - Carry the ACT-only restriction into every cognition claim.
